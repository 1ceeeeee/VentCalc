...
namespace VentCalc.Controllers {
    ...
    public class BuildingTypesController : BaseController {
        ...
        public async Task<IEnumerable<BuildingTypeResource>> ReadBuildingTypesByBuildingKind(int buildingKindId) {
            var buildingTypes = await UnitOfWork.Repository<BuildingType>()
                .GetEnumerableIcludeMultipleAsync(x => x.DeleteUsertId == null, x => x.BuildingPurpose);

            var buildingTypesByBuildingKind = buildingTypes
